...
#will take in value from -1 to 1
#will be mapped to -30 degrees to 30 degrees for each of the three
...
def pitch(pitch_angle):
    ...
    pitch_delta = np.matrix([pitch_delta_x, 0, pitch_delta_z])
    print(pitch_delta)
    return pitch_delta
...
def roll(roll_angle):
    ...
    roll_delta = np.matrix([roll_delta_x, 0, roll_delta_z])
    print(roll_delta)
    return roll_delta
...
def yaw(yaw_angle):
    ...
    yaw_delta = np.matrix([yaw_delta_x, 0, yaw_delta_z])
    print(yaw_delta)
    return yaw_delta

rf.ee_position()
rf.get_delta_q(0, 0, 2)
rf.ee_position()
